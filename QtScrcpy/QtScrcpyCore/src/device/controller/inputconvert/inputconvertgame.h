...
protected:
    ...
    void sendTouchDownEvent(int id, QPointF pos, float pressure = 1.0f);
    void sendTouchMoveEvent(int id, QPointF pos, float pressure = 0.0f);
    void sendTouchUpEvent(int id, QPointF pos, float pressure = 0.0f);
    void sendTouchEvent(int id, QPointF pos, AndroidMotioneventAction action, float pressure = -1.0f);
    ...
    //presure
    void startKeyClickPressureEnvelope(int sourceKey, int touchId, const QPointF &pos);
    void stopKeyClickPressureEnvelope(int sourceKey);
    // click mutil
...
private:
   ...
   struct ClickPressureState {
    int touchId = -1;
    QPointF pos;
    bool active = false;
    TapModel::PressureSeed pressureSeed;
    TapModel::OffsetSeed offsetSeed;
};
QHash<int, ClickPressureState> m_keyClickPressure;
};
