...
void InputConvertGame::sendTouchDownEvent(int id, QPointF pos, float pressure)
{
    sendTouchEvent(id, pos, AMOTION_EVENT_ACTION_DOWN, pressure);
}

void InputConvertGame::sendTouchMoveEvent(int id, QPointF pos, float pressure)
{
    sendTouchEvent(id, pos, AMOTION_EVENT_ACTION_MOVE, pressure);
}

void InputConvertGame::sendTouchUpEvent(int id, QPointF pos, float pressure)
{
    sendTouchEvent(id, pos, AMOTION_EVENT_ACTION_UP, pressure);
}

void InputConvertGame::sendTouchEvent(int id, QPointF pos, AndroidMotioneventAction action, float pressure)
{
...
// -------- key event --------
void InputConvertGame::startKeyClickPressureEnvelope(int sourceKey, int touchId, const QPointF &pos)
{
    ClickPressureState state;
    state.touchId = touchId;
    state.pos = pos;
    state.active = true;
    state.pressureSeed = TapModel::makePressureSeed();
    state.offsetSeed = TapModel::makeOffsetSeed();
    m_keyClickPressure[sourceKey] = state;

    const QList<int> sampleTimesMs = { 8, 16, 28, 40, 56, 72 };

    for (int tMs : sampleTimesMs) {
        QTimer::singleShot(tMs, this, [this, sourceKey, tMs]() {
            auto it = m_keyClickPressure.find(sourceKey);
            if (it == m_keyClickPressure.end() || !it->active) {
                return;
            }
            const float p = static_cast<float>(TapModel::pressureAt(static_cast<qreal>(tMs), it->pressureSeed));
            const QPointF offset = TapModel::offsetAt(static_cast<qreal>(tMs), it->offsetSeed);
            const QPointF samplePos = it->pos + offset;
            ...
            sendTouchMoveEvent(it->touchId, samplePos, p);
        });
    }
}
void InputConvertGame::stopKeyClickPressureEnvelope(int sourceKey)
{
    auto it = m_keyClickPressure.find(sourceKey);
    if (it != m_keyClickPressure.end()) {
        qDebug() << "[tap] stop"
                 << "key =" << sourceKey
                 << "touchId =" << it->touchId
                 << "pos =" << it->pos;
...
void InputConvertGame::processKeyClick(const QPointF &clickPos, bool clickTwice, bool switchMap, const QKeyEvent *from)
{
    ...
    const bool usePressureEnvelope = !clickTwice;

    if (QEvent::KeyPress == from->type()) {
        int id = attachTouchID(from->key());

        if (usePressureEnvelope) {
            // 第一帧不要直接打满，先给一个较小接触压力
            qDebug() << "[tap] key press"
                        << "key =" << from->key()
                        << "touchId =" << id
                        << "clickPos =" << clickPos;
            const TapModel::OffsetSeed downSeed = TapModel::makeOffsetSeed();
            sendTouchDownEvent(id, clickPos + TapModel::offsetAt(0.0, downSeed), 0.12f);
            startKeyClickPressureEnvelope(from->key(), id, clickPos);
        } else {
            // 双击先保持原逻辑
        ...
        if (usePressureEnvelope) {
            qDebug() << "[tap] key release"
                    << "key =" << from->key()
                    << "touchId =" << getTouchID(from->key())
                    << "clickPos =" << clickPos;
            stopKeyClickPressureEnvelope(from->key());
            sendTouchUpEvent(getTouchID(from->key()), clickPos, 0.0f);
        } else {
            sendTouchUpEvent(getTouchID(from->key()), clickPos);
        }
