...
#include <QTime>
#include <QTimer>
#include "ui/keymapeditordialog.h"
#include "config.h"
#include "dialog.h"
#include "ui_dialog.h"
#include "videoform.h"
...
void Dialog::initUI()
{
    ...
    auto editKeymapBtn = new QPushButton(tr("config script"), this);
    ui->horizontalLayout_8->addWidget(editKeymapBtn);  // 这就是 gameBox/refresh/apply 那一行的布局
    connect(editKeymapBtn, &QPushButton::clicked, this, [this]() {
        const QString serial = ui->serialBox->currentText().trimmed();
        auto device = qsc::IDeviceManage::getInstance().getDevice(serial);
        if (!device) {
            QMessageBox::warning(this, tr("Tip"), tr("No device selected/connected"));
            return;
    }

    // VideoForm 在连接设备时被塞进 device->setUserData(...)，断开时也用 getUserData() 取出来关闭:contentReference[oaicite:5]{index=5}
        auto data = device->getUserData();
        auto vf = static_cast<VideoForm*>(data);
        if (!vf) {
            QMessageBox::warning(this, tr("Tip"), tr("Video window not ready. Start service first."));
            return;
    }

        QImage img = vf->grabCurrentFrame();
        if (img.isNull()) {
            QMessageBox::warning(this, tr("Tip"), tr("Grab frame failed. Start service and try again."));
            return;
        }

        // 先做 MVP：弹窗显示截图（后续再换成可编辑的 QGraphicsView）
        QString keymapDir = QDir(QCoreApplication::applicationDirPath()).filePath("keymap");
        auto* editor = new KeymapEditorDialog(img, keymapDir, this);
        connect(editor, &KeymapEditorDialog::saved, this, [this](const QString& filename, const QString&){
            // 保存后自动刷新脚本列表
            on_refreshGameScriptBtn_clicked();
            // 下拉框选中刚保存的文件
            int idx = ui->gameBox->findText(filename);
            if (idx >= 0) ui->gameBox->setCurrentIndex(idx);
