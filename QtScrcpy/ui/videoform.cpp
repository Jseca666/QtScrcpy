...
#include <QImage>   // 如果没有的话

QImage VideoForm::grabCurrentFrame() const
{
    if (!m_videoWidget) return QImage();
    // m_videoWidget 是 QYUVOpenGLWidget（QOpenGLWidget子类），可以直接抓 framebuffer
    return m_videoWidget->grabFramebuffer();
}